...
def on_submit(doc, method=None):
    ...
    if not doc.get("mseller_ecf_type") or not doc.get("mseller_ecf_ncf"):
        if settings.require_ecf_fields_on_submit:
            frappe.throw(_("e-CF Type and e-NCF are required before submitting this Purchase Invoice."))
        return
    ...
    frappe.enqueue(
        ...
        job_id=f"mseller_ecf:Purchase Invoice:{doc.name}",
        deduplicate=True,
...
def on_cancel(doc, method=None):
    if doc.get("mseller_ecf_status") in {"Aceptado", "Aceptado Condicional"}:
        frappe.throw(
            _(
                "This purchase invoice has an accepted e-CF. Create the corresponding fiscal adjustment instead of cancelling it."
            )
        )
